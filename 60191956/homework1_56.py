...
previous_math_average, previous_english_average = map(float, input("수학과 영어의 이전 평균을 입력하시오: ").split())
previous_average = (previous_math_average + previous_english_average) / 2

#[1] 변수 선언 - 과목점수들의 총점과 평균 계산 및 출력
sum_math_score = float(math_score1 + math_score2 + math_score3)
sum_english_score = float(english_score1 + english_score2 + english_score3)
avg_math_score = sum_math_score / 3
avg_english_score = sum_english_score / 3
avg_score = (avg_math_score + avg_english_score) / 2
# 보고서 출력 [1]
print('\n' + ' - '*15 + '보고서' + ' - '*15)
print(f'이름 : {name:^30s}')
...
print(f'수학 평균 : {avg_math_score:.2f} 점')
print(f'영어 평균 : {avg_english_score:.2f} 점')
print(f'합산 평균 : {avg_score:.2f} 점')
