...
# 실습문제 1번 : 0을 입력 받을 때까지 수식 계산 후 출력
while(True):
    number = int(input('정수 입력: '))
    if number == 0:
        print('0을 입력하여 프로그램 종료', end='\n\n')
        break
    result = number**3 + 2*number + 5
    print(f'계산된 수식 결과는 {result}')

# 실습문제 2번 : 1~100까지 짝수만 출력
for even in range(2,100+1,2):
    print(f'{even},', end=' ')
print(end='\n\n')
