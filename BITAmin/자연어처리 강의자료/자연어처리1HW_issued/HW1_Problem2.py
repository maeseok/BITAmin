...
for i in range(size_context, len(test_sentence) - size_context):
    near = ([test_sentence[i-j-1] for j in range(size_context)] + [test_sentence[i+j+1] for j in range(size_context)])
...
class CBOW(nn.Module):
    def __init__(self, size_vocab, dim_embedding, size_context, hidden_size = 64):
        ...
        self.fclayer1 = nn.Linear(size_context * dim_embedding * 2, hidden_size)
        self.fclayer2 = nn.Linear(hidden_size, size_vocab)
    def forward(self, inputs):
        output = self.embed(inputs).view((1,-1))
        output = self.fclayer1(output).relu()
